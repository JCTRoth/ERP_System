...
[source,asciidoc]
----
image::flowchart.png[ERP System architecture flowchart,alt="ERP architecture flowchart",width=100%]
----

NOTE: The depicted flowchart is generated from `flowchart.xdot`.
* **Backend Services**:
...
The customer-facing webshop runs as a separate frontend application.
...
* Webshop: http://localhost:5174

image::images/webshop/home.png[Webshop home page in development,alt="Webshop home page in development",width=100%]

==== Webshop API Routing

* Requests to `/graphql` are proxied by Vite.
* The target is `process.env.GATEWAY_URL` or `http://localhost:4000`.
* Company context is sent via `X-Company-Id` from the webshop Apollo client.
...
All backend services seed demo data **by code** at startup (idempotent):

- `UserService` — super admin (`admin@erp-system.local`) + MediVita / Demo_Corporation demo users
- `ShopService` — MediVita products, customers, orders
- `MasterdataService` — currencies, tax codes, customers, departments, …
- `AccountingService` — chart of accounts, invoices, journal entries
- `OrdersService` — demo orders
- `CompanyService` — demo company (`DataInitializerConfig`), using the canonical demo company ID
- `NotificationService` — email templates (`EmailTemplateSeeder`)
- `TranslationService` — translations (Flyway SQL, generated from frontend locale files)

`scripts/dev/start-local.sh` calls `scripts/dev/seed-local.sh` after the backend is up.
If a service database is empty, `seed-local.sh` restarts that service so its startup
seeding re-runs (see `scripts/dev/README.md`).
